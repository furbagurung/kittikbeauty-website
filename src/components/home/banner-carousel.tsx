          ...
          <div className="mt-1.5 flex justify-center gap-1.5" aria-label="Banner slides">
            {visibleBanners.map((banner, index) => (
              <button
                type="button"
                key={`${banner.id}-dot`}
                className={`h-1.5 rounded-full transition-all ${index === activeIndex ? "w-5 bg-brandGold" : "w-1.5 bg-brandGold/35"}`}
...
function BannerSlide({ banner, priority }: { banner: Banner; priority?: boolean }) {
  ...
  const content = (
    <div className="relative aspect-[16/7.5] w-[calc(100vw-32px)] shrink-0 snap-center overflow-hidden rounded-[22px] border border-brandGold/25 bg-brandCream shadow-[0_12px_32px_rgba(0,69,31,0.08)] sm:aspect-[16/7] sm:w-[min(720px,72vw)] lg:w-[min(860px,64vw)]">
      <ProductImage
        src={banner.image}
      ...
      />
      {hasCopy ? (
        <div className="absolute inset-0 flex items-end bg-gradient-to-t from-brandEmerald/70 via-brandEmerald/10 to-transparent p-4 text-white sm:p-6">
          <div className="max-w-[76%]">
            ...
            {banner.cta ? (
              <p className="mt-2 inline-flex rounded-full bg-white/90 px-3 py-1 text-[11px] font-bold text-brandEmerald">
                {banner.cta}
              </p>
            ) : null}
          </div>
  ...
  return (
    <Link href={banner.link} className="block focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brandGold focus-visible:ring-offset-4">
      {content}
    </Link>
  );
}
